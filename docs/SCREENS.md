...
- **개별 팩 게시 링크(공유 링크) 기능은 폐지되었습니다** — 전달은 **리포트 HTML 내보내기(선택적 열람 암호)** 로 일원화되었습니다.
...
> - **전달 방법** ① **리포트 HTML 내보내기**(메인, **선택적 열람 암호**로 AES-256-GCM 암호화) · ② 이메일 전달(mailto). *(개별 팩 게시 링크 `#share`는 폐지)*
...
- **열람 암호(선택)** 전달 시 암호를 입력하면 리포트 전체를 **AES-256-GCM**(PBKDF2-SHA256)으로 암호화합니다 — 파일이 유출돼도 암호 없이는 열람 불가(확산 방지). 열 때 브라우저(WebCrypto)에서 복호화하며, 암호는 파일과 **다른 경로로** 전달합니다.
